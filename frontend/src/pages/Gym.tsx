...
export default function Gym() {
  ...
  // NUEVO: Función inteligente para calcular y guardar calorías
  const handleToggleSession = () => {
    if (sessionActive) {
      // Fórmula: 150 kcal base por sesión + 0.08 kcal por cada kg levantado
      const caloriasCalculadas = Math.round(150 + (volume * 0.08));
      
      // Guardamos en la persistencia local
      const guardado = localStorage.getItem('caloriasQuemadas');
      const totalActual = guardado ? parseInt(guardado) : 0;
      localStorage.setItem('caloriasQuemadas', (totalActual + caloriasCalculadas).toString());
      
      alert(`¡Entrenamiento completado! Has movido ${volume}kg y quemado ${caloriasCalculadas} kcal 🔥`);
      setVolume(0); // Reiniciamos el contador
    }
  ...
  return (
    <div className="p-6 space-y-6 animate-fadeIn">
      ...
      <div className="space-y-4">
        ...
        {exercises.map((ex) => (
          <div key={ex.id} className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4 transition-all">
